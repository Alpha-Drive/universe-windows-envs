...
public:
	...
	~Rewarder();
	void sendRewardAndIncrementStepCounter(const double reward, const bool done, const Json::Value& info=Json::Value(Json::ValueType::objectValue));
	...
	void hard_reset();
private:
	void sendReward_(const double reward, const bool done, const Json::Value& info=Json::Value(Json::ValueType::objectValue));
	void sendReset_();
