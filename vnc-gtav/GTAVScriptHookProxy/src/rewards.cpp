...
//			BOOST_LOG_TRIVIAL(info) << "Road map via paths.xml not loaded, loading now...";
//			std::string gtav_dir = std::getenv("GTAV_DIR");
//			std::string paths_xml = gtav_dir + "\\paths.xml";
//			BOOST_LOG_TRIVIAL(info) << "Loading from " << paths_xml;
//			populateNodes(paths_xml.c_str());
//			PATHFIND::LOAD_ALL_PATH_NODES(FALSE);
//			BOOST_LOG_TRIVIAL(info) << "Successfully loaded paths.xml";
//		}
//		else return 0.0;
//	}
//
